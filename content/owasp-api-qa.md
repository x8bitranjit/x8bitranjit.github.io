...
Two-account differential: as user A, capture a request touching object `A1`; replay with **B's token** against `A1` — blocked? Then swap IDs everywhere: path, query, body, headers (`X-Account-Id`), nested JSON, GraphQL `node(id:)`. Try victim, adjacent, and enumerated IDs. Test **all verbs** (GET disclosure; PUT/PATCH/DELETE tamper/destroy). Automate with Autorize.
...
`alg:none`; weak/guessable HS256 secret (crack offline); RS256→HS256 key confusion; `kid`/`jku`/`x5u` injection; missing signature verification; missing/`exp` too long; claim tampering (`sub`/`role`). Any → forge tokens → impersonate. → `../Web/JWT/`.
...
Hammer an endpoint → is there a per-user/IP **rate limit**? Send huge payloads / huge pagination (`limit=99999999`) / unbounded batch. Trigger **amplification** (deep GraphQL nesting, aliased/batched queries, expensive filters/exports). Hit cost-driving actions (SMS/email/paid-API) and quantify $/request. → `../Web/RaceCondition/` (rate-limit method), `GraphQL/` (complexity).
...
Find server-fetch sinks (`webhook_url`, `callback`, `image_url`, `import_url`, `?url=`, SSO metadata/JWKS, XML→XXE). Point them at metadata/localhost/internal → creds/internal responses. For **blind**, use an **OOB** listener (Collaborator/interactsh) — DNS/HTTP callback confirms the server fetched it. → `../Web/SSRF/`.
...
Enumerate all versions/endpoints (`/v1../v2../v3`, `/api../api-internal../beta`, route/param brute → `../Web/Recon/`); mine JS bundles, mobile decompile, Swagger/OpenAPI, **GraphQL introspection**, Wayback, git history, error messages (→ `../Web/JSFiles/`, `GraphQL/`); find non-prod hosts (staging/dev/uat subdomains → `../Web/SubdomainTakeover/`); probe debug/internal (`/debug`, `/actuator`, `/metrics`).
