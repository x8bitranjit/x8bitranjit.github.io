...
- **Class-defining research** — **Nick Aleks & Dolev Farhi, _Black Hat GraphQL_** (No Starch Press, 2023) — the definitive offensive-GraphQL text (introspection recovery, BOLA/BFLA, alias/array batching brute-force, DoS, resolver injection, CSWSH); Doyensec **InQL** research; disclosed HackerOne GraphQL reports (batching → OTP/2FA brute → ATO, `node`/`*ById` cross-user BOLA, SSRF via url-taking mutations).
...
- **CWE / CVSS (standards)** — CWE-639/285/862/915 (authz/mass-assign) · 89/943/77/78 (injection) · 918 (SSRF) · 200 (info) · 352 (CSRF) · 770/400 (DoS) · 1385 + CSWSH (WebSocket hijacking); score with **CVSS 3.1** (FIRST calculator).
