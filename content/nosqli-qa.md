# NoSQL Injection — Zero to Expert (120 Q&A)
...
**Q101. Explain NoSQL injection to a non-technical person in three sentences.** A login form quietly asks the database "find a user whose username is X and password is Y." Normally you type a plain password into the Y slot — but if the app is careless, you can type a little **command** instead, like "any password that isn't empty." The database obeys the command, decides "yes, that matches," and lets you in with no real password.

**Q102. What's the single difference from SQL injection that changes your whole toolkit?** In SQLi you break out of a **string** in a text query (quotes, `UNION`, `--` comments). In NoSQLi you usually inject **structure** — a JSON **operator** (`$ne`, `$regex`) where the app expected a scalar — so there's no comment character and no `UNION`; you flip booleans with operators and extract data with `$regex`/`$where` oracles. The mental model shifts from "escape the quote" to "turn a value into an object."

**Q103. Why does `{"password":{"$ne":null}}` log you in, mechanically?** The login runs `findOne({username, password})`. When the app merges your JSON body straight into that filter, `password` is no longer the string it expected — it's the operator object `{$ne:null}`, meaning "password field is not null," which is true for essentially every user. So the query returns the first matching document and the app, treating "a document came back" as "credentials valid," issues a session. The password check was replaced by an always-true condition.

**Q104. How can a plain HTML form — not JSON — still be NoSQL-injectable?** Because frameworks auto-parse **bracket notation**. Express's `qs` and PHP turn a form/query field `user[$ne]=x` into the nested object `{user:{$ne:"x"}}` before the app ever sees it — so `username[$ne]=x&password[$ne]=x` becomes operator injection without any JSON. That's why you always test both encodings: a sanitizer that only guards the JSON path often misses the bracket path.

**Q105. Walk me through turning a blind true/false signal into account takeover.** It's a game of twenty questions with `$regex`. You find any response that differs for a true vs false query (login OK/fail, a "valid":true flag, a result count), then ask "does the secret start with 'a'?" via `{"$regex":"^a"}`, read the yes/no, and extend one character at a time — binary-searching the charset. Point it at a **password-reset token** and you rebuild the whole token, complete the reset, and take over the account. That's the Rocket.Chat chain (CVE-2021-22911), unauthenticated.

**Q106. When is NoSQLi actually RCE, and when is it "just" auth bypass?** Most NoSQLi tops out at **auth bypass / ATO** — operator injection flips booleans, blind `$regex` dumps secrets. It becomes **RCE** only through a specific sink: a server-side **JavaScript** context (`$where`/`mapReduce`/`$function`, or the app piping "JS" into Node `eval`), or a **datastore-specific** path (Elasticsearch script CVE-2015-1427, Redis `CONFIG SET`+`SAVE` webshell, CouchDB dup-key admin + CVE-2017-12636, Neo4j `apoc`). Always confirm the exact sink before claiming a shell — on modern MongoDB, `$where` JS is sandboxed (no shell/network).

**Q107. A developer says "we use Mongoose with a schema, so we're safe from NoSQLi." Rebut it.** A schema helps but isn't a guarantee: Mongoose casts *typed* fields, but `Mixed`/`Object` fields, `.find(req.query)` passthroughs, `$where`, and raw aggregation stages still accept operators, and casting can be bypassed on some versions. The robust fix is to **type-check every query input to string/number and reject objects/arrays**, compare passwords in application code (not inside the query), and recursively strip `$`-keys. "We have a schema" is a lead to verify, not a defense to trust.

**Q108. What's the parser-differential / duplicate-key class, and why does it matter?** It's NoSQLi that isn't about operators at all — it exploits **two JSON parsers disagreeing**. CVE-2017-12635 (CouchDB) sent a `_users` doc with **two `roles` keys**; one parser used the last key to authorize the write, another used the first for later authorization, so a non-admin granted themselves `_admin`. It's the NoSQL cousin of HTTP parameter pollution: when a document write is authorized by one interpretation and acted on by another, you get privilege escalation. Test duplicate keys and type juggling, not just `$ne`.

**Q109. Why is the password-reset endpoint your highest-value NoSQLi target?** Three reasons: it's often **unauthenticated** (pre-auth attack surface), it holds the **highest-value secret** (a token that *is* an account), and it frequently takes a JSON object for the token/email — perfect for `$regex` extraction. A blind NoSQLi there leaks the reset token char-by-char → ATO of any account including admin, with no credentials. That's exactly why CVE-2021-22911 was critical; always decode and fuzz reset/verify/policy endpoints.

**Q110. What's your SAFE-PoC discipline for NoSQLi?** Use **two of my own accounts** and control-baseline every test (a control request vs the injected one) to keep false positives near zero. For blind extraction, extract a secret **I own** (my second account's reset token), bound the loop, and redact it in the report — never dump real users' data. For `$where`, prove the JS sink with a benign predicate/timing, not a DoS or an `eval` shell on prod. One proof — logged in as B, or one extracted-own token — then stop.

**Q111. Rapid-fire curveballs: (a) the app strips `$` from keys — dead? (b) `$regex` extraction is slow — speed it up? (c) it returns the same response whether the query matched or not — any hope?** (a) Not necessarily — try nested (`{"$not":{"$eq":…}}` reconstructed), bracket-path, unicode/alternate `$`, or a place the sanitizer doesn't recurse (arrays, deep nesting). (b) Binary-search the charset (`$regex` ranges like `^[a-m]`) instead of linear, and discover length first. (c) Look for **timing** (`$where` with a `sleep`) or a subtle side-channel (response length, a downstream email/action) — a boolean oracle doesn't have to be a visible message.
...
**Q112. Scenario: a JSON login returns 401 for `{"password":"x"}` but 200 for `{"password":{"$ne":"x"}}`. What do you have and what next?** Confirmed operator injection — the differential (string fails, operator flips it) is control-baselined proof of NoSQLi, not a fluke. Next: make both fields always-true (`{"username":{"$ne":null},"password":{"$ne":null}}`) to land as the first user (often the seed admin); if that lands non-admin, pin `{"username":"admin","password":{"$ne":""}}` or blind-extract admin creds. Severity Critical (auth bypass to admin). Confirm behavior change (you reach an authed/admin-only page), not just a 200.

**Q113. Scenario: login is patched, but a search endpoint `GET /orders?userId=<me>` still uses your input in a Mongo filter. How do you attack it?** Turn the restrictive filter permissive: `userId[$ne]=0` (or `[$gt]=`) to make "only my orders" return **everyone's** orders — a data-disclosure / authz bypass (pairs with IDOR). Then widen searches with `search[$regex]=.*`, and probe other filters (`published[$ne]=true` for drafts, `isDeleted[$ne]=false`). Severity High (cross-user data exposure); quantify with a bounded sample, don't dump the collection.

**Q114. Scenario: you find a blind boolean on `/api/reset/verify` (`{"valid":true/false}`) that reflects a token check. Walk the exploit and the severity.** This is the CVE-2021-22911 shape and the highest-value NoSQLi. Blind-extract the victim's reset token with `{"email":"victim@x","token":{"$regex":"^…"}}`, one char at a time (binary-search `0-9a-f`, discover length via `^.{32}$`), automate with `nosqli_blind.py`. With the full token, complete the reset and log in = **ATO** (admin's email → admin → often RCE via webhook). Critical, and typically **unauthenticated**. Prove it on my own second account, bounded, token redacted.

**Q115. Scenario: `$regex` is filtered (stripped from keys), but the app uses `$where` somewhere. Now what?** Pivot to `$where` JavaScript extraction: `{"$where":"this.token.match(/^7/)!=null"}` or `{"$where":"this.password[0]=='a'"}` gives the same char-by-char oracle without `$regex`. If even direct value output is invisible, use a **timing** oracle: `{"$where":"if(this.token[0]=='7'){sleep(3000)};true"}` and read the delay. Confirm whether the `$where` JS is sandboxed (modern Mongo: extraction/timing only) or reaches `eval` (→ RCE) before you claim a shell.

**Q116. Scenario: the target is Apache CouchDB and you can hit the `_users` database. What's the specific attack?** Test **CVE-2017-12635** — the duplicate-key privilege escalation. Submit a `_users` document with **two `roles` keys** (e.g. `{"type":"user","name":"x","roles":["_admin"],"roles":[],"password":"y"}`): the parser inconsistency authorizes the write with one and applies the other, granting your new user `_admin`. That's privilege escalation to admin; chained with CVE-2017-12636 it's RCE as the DB user. Confirm the version (<1.7.1 / <2.1.1), create a throwaway user, prove admin, and stop.

**Q117. Scenario: a sanitizer blocks any request where a key starts with `$`. How do you bypass it and what's the lesson?** Try the paths it doesn't walk: **bracket/dot notation** (`user[$ne]=` if it only checks JSON, or `user.$ne`), **nested operators** it reconstructs (`{"$not":{"$eq":null}}`), **arrays / deep nesting** where the recursive stripper stops early, and **unicode/alternate encodings** of `$`. The lesson for the report: blocklisting `$` keys is fragile — the fix is **type-checking values to scalars** (reject objects entirely) plus recursive schema validation, not string-matching keys.

**Q118. Scenario: you confirm `$where` runs JS on MongoDB 3.6. Is it RCE?** Almost certainly **not a shell** — MongoDB's `$where` JS runs in a sandboxed SpiderMonkey/V8 with no `require`, no `child_process`, no network. So it's **blind data extraction + timing + DoS**, which is still High/Critical via ATO, not RCE. It *becomes* RCE only if the app itself pipes the "JS" into a Node `eval`/`vm` outside Mongo, or on legacy `db.eval()` (removed in 4.2). Report it as JS-injection blind exfil unless you can actually demonstrate code execution outside the sandbox.

**Q119. Scenario: NoSQLi lets you dump a `userPassword` field but it's a bcrypt hash. Worth reporting, and what's the real move?** Yes — readable credential material is a finding, but the *hash* is Medium-ish alone (bcrypt resists cracking). The higher-impact move is to extract a **reset token / session token / 2FA seed** instead (directly usable = ATO, no cracking), or use the same blind oracle to confirm a **known** password and prove the auth model is broken. Report the disclosure, but lead with the ATO path (token → account) rather than "I have a hash," which triagers rate lower.

**Q120. Scenario: the app is behind a WAF that flags `$ne`/`$regex` in bodies, and your operators get 403'd. How do you still land the finding?** The WAF is signature-matching the literal operators, so avoid the literal: use **bracket/form encoding** (`password[$ne]=x`) which many WAFs don't normalize, **deep-object `qs`** notation (`filter[a][b][$ne]=`), **nested/alternate operators** (`$not`/`$nin`), unicode-escaped `$`, or move the operator into a field the WAF doesn't inspect (a header/param that also reaches the query). Baseline against a control so you know when a variant slips through, and if all bodies are inspected, pivot to a different injectable parameter — WAFs rarely cover every input.
