# OAuth / OIDC / SAML — Zero to Expert (120 Q&A)
...
**Q101. Explain OAuth vs OIDC vs SAML to a non-expert in a few sentences.** All three are the **"Sign in with…"** button — the app outsources the ID check to a trusted provider (Google/Apple/Okta) and trusts the answer. **OAuth 2.0** is really about *authorization* (a token that grants access to an API); **OIDC** is a thin layer on top that adds *authentication* (an `id_token` — a JWT saying "this is alice"); **SAML** is the older enterprise version that does the same job with a big signed XML "assertion" instead of a JWT. Same picture — a signed ID card handed from the provider to the app — different envelope.

**Q102. Why do SSO bugs pay more than most auth bugs?** Because a single flaw is **account takeover, pre-authentication, usually bypassing 2FA, and often across many downstream apps.** The IdP is the root of trust: if you can steal, forge, confuse, or replay the identity assertion, you're the victim to *every* app that trusts that IdP — and 2FA lives at the IdP, which you've sidestepped. "Sign in with Apple" was one bug that threatened Dropbox, Spotify, Airbnb and Giphy at once.

**Q103. What's the #1 OAuth bug and the mechanism behind it?** Sloppy **`redirect_uri`** validation. The IdP mails the authorization `code`/`token` to the `redirect_uri`; if the app's allow-list check is loose (suffix not anchored, `@`-userinfo confusion, substring match, subdomain wildcard, parameter pollution), you point it at a host you control and the IdP hands the victim's code straight to you → redeem → ATO. It's #1 because it's easy to get subtly wrong and the payoff is a full takeover.

**Q104. The `redirect_uri` is a strict exact-match. Is the redirect attack dead?** No — pivot to an **open redirect on the *allowed* origin**. The IdP only checks the host is `app.example.com`; if that host has *any* endpoint that forwards the browser off-origin (`/out?url=`, a `next=` param, even a `Referer` leak to a third-party resource on the callback page), the code lands on the allowed host and then bounces to you. That chain is the single most common way strict `redirect_uri` gets broken in the real world.

**Q105. What does `state` protect against, and what's the classic exploit when it's missing?** `state` is the OAuth CSRF token — it binds the callback to the browser that *started* the flow. Missing/unchecked `state` enables **account-linking CSRF**: you capture your *own* "link Google" code, don't complete it, and feed the callback to a logged-in victim; their browser links **your** Google identity to **their** account, and now you sign in via Google into their account. It's the highest-value `state` finding — a silent ATO with one victim interaction.

**Q106. Walk me through the "Sign in with Apple" bug — why was a valid signature not enough?** Apple would issue a correctly-signed `id_token` for *any* email the client asked for. Apps verified the signature against Apple's public key, saw "valid," and logged the user in by the `email` claim — but the signature only proves *Apple issued it*, not *who was authenticated*. So an attacker requested a token asserting the victim's email, and every app that trusted "valid Apple token for this email = this user" was taken over. The lesson: **verify the signature *and* pin who's allowed to assert the identity** (`iss`/`aud`, and don't trust a client-chosen email).

**Q107. What's PKCE and when does its absence matter?** PKCE binds the authorization request to the token exchange: the client sends a hashed secret (`code_challenge`) up front and must present the matching `code_verifier` at `/token`, so a stolen `code` is useless without the verifier. It matters most for **public clients** (SPAs, mobile) that can't keep a client secret — without PKCE (or with a downgradeable/omittable PKCE), a stolen code redeems directly to tokens. Always test whether PKCE is enforced or can be downgraded (`plain`) or dropped.

**Q108. Compare OAuth token theft with SAML forgery — different mechanics, same outcome?** Same outcome (ATO), different card. OAuth/OIDC theft is about **stealing the code/token in transit** (redirect_uri, referer, fragment) or **forging a JWT** (`alg:none`, unpinned `aud`, `kid`/`jku`). SAML is a signed XML assertion, so the attacks are XML-specific: **signature wrapping (XSW)** (add a forged assertion the app reads while the signature covers the original), **signature stripping** (unsigned accepted), **comment injection** (canonicalization strips comments but the parser truncates the NameID). Both come down to "make the app accept an identity it shouldn't."

**Q109. What's "pre-account-takeover" and why is it so nasty?** The attacker pre-registers the victim's email (unverified) on the app; later the victim signs up via SSO and the app **merges** the SSO identity into the attacker's existing record instead of creating a fresh account — so the attacker keeps access. It's nasty because it's often **zero-click** for the victim, survives password resets, and hides for months. USENIX 2022 found 35 of 75 major services vulnerable. The fix is to require email verification before any link/merge.

**Q110. How do you demonstrate an SSO ATO ethically?** Two accounts you own — attacker and victim — and capture only your *own* assertions to rehearse. For redirect theft, use your own collector host and steal *your second account's* code, then log in as it and screenshot `/api/me`. For forgery/linking, prove "as attacker I reached account B" and stop; never capture a real user's code, never forge a real identity, tear down collectors. One benign "I'm inside B" proof is the whole report.

**Q111. Rapid-fire: (a) the app shows the OAuth code in the URL but strips it fast — exploitable? (b) SAML `Response` is signed — is the assertion safe? (c) `id_token` has `aud` = some other client — accept it?** (a) Maybe — a third-party resource (analytics/`<img>`) loading *before* the strip leaks the code via `Referer`; check load order. (b) Not necessarily — signature may cover the `Response` but not the `Assertion` (or vice-versa); XSW exploits exactly that gap, so verify **what** is signed. (c) No — that's **audience confusion**; an app must reject a token whose `aud` isn't its own client_id (and check `azp` with multiple audiences), or a token minted for another client logs you in.

---

## Level 11 — Scenario-based questions

> *You're handed a situation — decide the next move and the severity. Worked, concrete answers.*

**Q112. Scenario: `redirect_uri=https://app.example.com@evil.com/cb` is accepted by the IdP. What do you have and what's the severity?** The `@` makes `evil.com` the real host (userinfo confusion, same trick as SSRF) — so the IdP mails the victim's `code`/`token` to your server. Stand up a collector, send the victim your crafted "Sign in" link, capture their code, redeem it (if public/PKCE-less) → ATO. Critical if it lands a usable code/token. Prove with your own second account; if PKCE blocks redemption, the leaked code alone may still be reportable as High (assertion disclosure) — note the PKCE dependency.

**Q113. Scenario: `redirect_uri` is strict exact-match, but you find `app.example.com/out?url=` is an open redirect. Chain it.** Set `redirect_uri` to a callback on the allowed host that forwards through `/out` to your collector (`…/oauth/callback?next=https://app.example.com/out?url=https://you`). The IdP sees host `app.example.com` and accepts; the code lands there and bounces to you carrying `?code=`. Redeem → ATO. This is the "strict redirect_uri + open redirect on allowed origin" chain — High/Critical, and the most common real-world break of strict validation.

**Q114. Scenario: the site has "Connect your Google account" and you notice the callback works even after you delete `state`. Exploit?** Missing/ignored `state` + account-linking = **silent ATO**. Start the "link Google" flow with *your* attacker Google account, capture your `code` but don't complete it, craft the callback URL, and get a logged-in victim to load it (one click). Their session links *your* Google identity to *their* account; you then "Sign in with Google" into their account. Critical. Demonstrate attacker→B with two own accounts.

**Q115. Scenario: an app logs users in via a signed Apple/Google `id_token`. What do you check to find a forgery bug?** Verify how the SP validates the token: does it check the **signature against the IdP's `jwks_uri`**, **pin `iss` and `aud`** to the expected values, **reject `alg:none`**, enforce `nonce`, and — crucially — **not trust a client-supplied `email` as identity without `email_verified`**? The "Sign in with Apple" bug was a valid signature on an attacker-chosen email; so try substituting the `email`/`sub` to a victim and see if the app authenticates you. Any missing check → forge identity → ATO (Critical).

**Q116. Scenario: you can inject an XML comment into the SAML `NameID` and the signature still validates. What's the bug and impact?** SAML **comment injection** (the Duo class, CVE-2017-11427 et al.): canonicalization strips the comment before signature check, so `admin@target.com<!---->.attacker.com` stays validly signed, but the SP's DOM API reads it as `admin@target.com` (text after the comment dropped). You authenticate as `admin@target.com` without a valid credential → auth bypass / ATO. Critical. Confirm the SP's SAML library/version is in the vulnerable set and that it parses NameID with the unsafe API.

**Q117. Scenario: the SAML `Response` element is signed, but you can add a second, unsigned `Assertion`. Describe the attack.** **XML Signature Wrapping (XSW).** You keep the original signed assertion (so the signature verifies) but add a forged, unsigned assertion the application logic actually reads — exploiting the gap between *what's signature-verified* and *what's processed*. Move/wrap elements so the verifier checks the original but the SP consumes your `NameID=admin@target`. Result: admin session on a validly-"signed" message. Critical; test the 8 XSW permutations, since which one works depends on the SP's parsing order.

**Q118. Scenario: `id_token` validates fine, but its `aud` claim is a *different* client_id than the app's. Accept or reject?** Reject — this is **audience confusion / confused deputy**. A token minted for client X should never authenticate you to client Y; the SP must verify `aud` == its own `client_id` (and, with multiple audiences, that `azp` matches). If the app accepts it, a token you obtained legitimately for *your* app logs you into a *different* app as the victim. Report it as a token-audience validation flaw (Critical if it yields cross-app ATO).

**Q119. Scenario: an SSO app takes a `request_uri` parameter and fetches it server-side. What do you test beyond the auth flow?** **SSRF.** `request_uri` (JAR/PAR) makes the IdP fetch a URL you supply — point it at `http://169.254.169.254/latest/meta-data/…` (cloud metadata → IAM creds) or an internal service and watch for the fetch/OOB. Same for a SAML parser that resolves external entities (**XXE**) → file read (`/etc/passwd`, private keys) or blind OOB. These are High/Critical infrastructure findings *in addition to* the auth bug — cross-ref the SSRF/XXE kits.

**Q120. Scenario: you achieve implicit-flow with `access_token` in the URL fragment, and there's an open redirect on the client. Is the fragment reachable?** Yes — fragments (`#access_token=…`) **survive redirects** (the browser re-attaches the fragment to the new location), so an open redirect that doesn't strip it ships the token to your host. Exfiltrate the `access_token`, call the API/userinfo as the victim → ATO/data access (High). This is why implicit flow is discouraged; note the fragment-preserving redirect as the exfil primitive and recommend the code+PKCE flow instead.
